...
from firewallxpl.core.exploit.printer import (
    print_info,
    print_success,
    print_error,
    print_status,
...
from firewallxpl.core.exploit.exploit import GLOBAL_OPTS
from firewallxpl.core.exploit.payloads import BasePayload
...
class BaseInterpreter:
    ...
    def get_command_handler(self, command):
        ...
        try:
            ...
            raise FirewallXPLException("Unknown command: '{}'".format(command))

        return command_handler

    def start(self):
        """ FirewallXPL main entry point. Starting interpreter loop. """
        ...
        while True:
            ...
            except FirewallXPLException as err:
                print_error(err)
            except (EOFError, SystemExit):
                print_info()
                print_error("FirewallXPL stopped")
                os._exit(0)
...
class FirewallXPLInterpreter(BaseInterpreter):
    ...
    def __init__(self):
        super(FirewallXPLInterpreter, self).__init__()
        PrinterThread().start()
...
""".format(exploits_count=self.modules_count["exploits"],
    ...
    def __parse_prompt(self):
        ...
        raw_prompt_template = os.getenv("FXF_RAW_PROMPT", raw_prompt_default_template).replace('\\033', '\033')
        ...
        module_prompt_template = os.getenv("FXF_MODULE_PROMPT", module_prompt_default_template).replace('\\033', '\033')
    ...
    def command_use(self, module_path, *args, **kwargs):
        ...
        module_path = ".".join(("firewallxpl", "modules", module_path))
