...
LOCAL vs PHYSICAL) and indicates which ones have FirewallXPL-Forge exploit
...
class Exploit(Exploit):
    __info__ = {
        ...
        "description": "Queries the embedded CVE database for known vulnerabilities "
                       ...
                       "PHYSICAL. Lists available FirewallXPL-Forge exploit modules. "
    ...
    def run(self):
        ...
        if remote_exploitable:
            print_success("=== EXPLOITABLE by FirewallXPL-Forge ({}) ===".format(len(remote_exploitable)))
            print_status("")
        ...
        if non_remote and self.show_physical:
            ...
            print_info("  (FirewallXPL-Forge cannot exploit these remotely)")
            print_status("")
