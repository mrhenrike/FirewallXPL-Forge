...
Uses the framework's own Ruby modules *verbatim* — no FirewallXPL port of MSF
...
class Exploit(Exploit):
    ...
    msfconsole_path = OptString(
        "",
        "Path to msfconsole binary (empty = FXF_METASPLOIT_CONSOLE env or PATH)",
    )
    ...
    def run(self) -> None:
        ...
        if not bin_path:
            print_error(
                "msfconsole not found. Install Metasploit or set msfconsole_path / FXF_METASPLOIT_CONSOLE.",
                verbose=True,
            )
            return
