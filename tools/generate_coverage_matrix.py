...
def _matrix_source_revision(repo_root: Path) -> str:
    """Return git tree id for firewallxpl/modules (stable for a checkout; not the commit hash)."""
    try:
        completed = subprocess.run(
            ["git", "rev-parse", "HEAD:firewallxpl/modules"],
            cwd=str(repo_root),
            capture_output=True,
            text=True,
            timeout=8,
...
def _osi_tcpip_coverage_section(records: List[ModuleRecord], repo_root: Path) -> str:
    ...
    catalog_path = repo_root / "firewallxpl" / "resources" / "catalogs" / "osi_tcpip_priority_matrix.json"
...
def _market_priority_section(records: List[ModuleRecord], repo_root: Path) -> str:
    ...
    catalog_path = repo_root / "firewallxpl" / "resources" / "catalogs" / "market_priority_devices_2010_2026.json"
    if not catalog_path.exists():
...
def _external_intel_section(repo_root: Path) -> str:
    ...
    source_file = repo_root / "firewallxpl" / "resources" / "catalogs" / "external_tool_intel_sources.json"
...
def _discord_requested_section(records: List[ModuleRecord], repo_root: Path) -> str:
    ...
    catalog_path = repo_root / "firewallxpl" / "resources" / "catalogs" / "discord_requested_devices.json"
    if not catalog_path.exists():
...
def _architecture_inventory_section(repo_root: Path) -> str:
    ...
    index_path = repo_root / "firewallxpl" / "resources" / "catalogs" / "arsenal_index.json"
...
def _workspace_reuse_inventory_section(repo_root: Path) -> str:
    ...
    inv_path = repo_root / "firewallxpl" / "resources" / "catalogs" / "workspace_reuse_inventory.json"
...
def _deep_intel_backlog_section(repo_root: Path) -> str:
    ...
    backlog_path = repo_root / "firewallxpl" / "resources" / "catalogs" / "deep_intel_backlog.json"
...
def _build_summary(records: List[ModuleRecord], matrix: Dict[Tuple[str, str], CoverageEntry]) -> str:
    ...
    lines: List[str] = [
        ...
        "",
        f"- Module tree (firewallxpl/modules): {_matrix_source_revision(REPO_ROOT)}",
...
def main() -> int:
    ...
    _configure_logging()
    repo_root = Path(__file__).resolve().parent.parent
    modules_root = repo_root / "firewallxpl" / "modules"
