...
def _disk_snapshot(repo_root: Path) -> Dict[str, Any]:
    """Aggregate byte sizes by top-level path and under ``firewallxpl/``."""

    repo_root = repo_root.resolve()
    rx_root = repo_root / "firewallxpl"
    ...
    for dirpath, dirnames, filenames in os.walk(repo_root):
        ...
        for name in filenames:
            ...
            files_rx += 1
            if rel_rx == Path("."):
                totals_rx["(firewallxpl root files)"] += sz
            else:
    ...
    return {
        ...
        "firewallxpl_files_count": files_rx,
        "repo_by_top": dict(totals_repo),
        "firewallxpl_by_top": dict(totals_rx),
...
def _first_party_py_counts(repo_root: Path) -> Dict[str, int]:
    ...
    rx = repo_root / "firewallxpl"
    counts: Dict[str, int] = {}
    for name in ("core", "modules", "libs"):
        p = rx / name
        counts["firewallxpl/{}".format(name)] = _count_py_files(p)
...
def _build_md(
    ...
    lines.extend([
        ...
        "| Files under ``firewallxpl/`` | {} |".format(disk.get("firewallxpl_files_count", 0)),
        "",
    ...
    for name, sz in _sorted_sizes(disk.get("firewallxpl_by_top") or {}, limit=25):
        pct = (100.0 * sz / gt) if gt else 0.0
        lines.append("| `{}` | {} | {:.1f}% |".format(name, _human_bytes(sz), pct))
    ...
    for key in ("firewallxpl/core", "firewallxpl/modules", "firewallxpl/libs", "tools", "rxf.py"):
        if key in py_counts:
    ...
    try:
        from firewallxpl.core.cve.cve_db import CVEDatabase
        db = CVEDatabase()
        lines.extend([
            "",
...
def _build_txt(
    ...
    lines.extend(["", "firewallxpl/ first-level folders:",])
    for name, sz in _sorted_sizes(disk.get("firewallxpl_by_top") or {}, limit=25):
        pct = (100.0 * sz / gt) if gt else 0.0
        lines.append("  {:<36} {:>12}  ({:.1f}%)".format(name, _human_bytes(sz), pct))
    res_ch = disk.get("resources_children") or {}
    if res_ch:
        lines.extend(["", "firewallxpl/resources/* direct children (largest):",])
        for name, sz in _sorted_sizes(res_ch, limit=25):
            pct = (100.0 * sz / gt) if gt else 0.0
            lines.append("  {:<36} {:>12}  ({:.1f}%)".format(name, _human_bytes(sz), pct))
