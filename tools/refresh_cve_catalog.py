#!/usr/bin/env python3
"""Regenerate firewallxpl/resources/catalogs/cve_extended_catalog.json from curated rows.
...
(3) CVE strings found under ``firewallxpl/modules/**/*.py`` that are not already covered
...
def _vendor_product_from_module_path(rel_posix: str) -> Tuple[str, str]:
    ...
    return "firewallxpl", parts[0]


def _entries_from_external_intel(repo_root: Path, seen: Set[str]) -> List[Dict[str, Any]]:
    path = repo_root / "firewallxpl" / "resources" / "catalogs" / "external_tool_intel_sources.json"
    if not path.is_file():
        return []
    try:
...
def _entries_from_modules(repo_root: Path, seen: Set[str]) -> List[Dict[str, Any]]:
    mods = repo_root / "firewallxpl" / "modules"
    if not mods.is_dir():
        return []
    out: List[Dict[str, Any]] = []
    for py in sorted(mods.rglob("*.py")):
        ...
        for m in _RX_CVE.findall(text):
            ...
            desc = (
                "CVE citado no módulo FirewallXPL ``{}``. "
...
def _discord_related_cve_hints(repo_root: Path) -> Set[str]:
    path = repo_root / "firewallxpl" / "resources" / "catalogs" / "discord_requested_devices.json"
    if not path.is_file():
        return set()
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
...
def _merge_tg12_poc_refs_for_scope(
    ...
    for cve in sorted(scope_cves):
        ...
        neo = row(
            ...
            "PoC listado no índice tg12 para CVE em âmbito FirewallXPL; validar impacto e licença.",
            0.0,
            exploit_available=True,
        )
...
def main() -> int:
    ...
    from firewallxpl.core.cve.cve_db import _EMBEDDED_CVES
    ...
    payload = {
        "catalog_note": (
            "Índice estendido offline para FirewallXPL-Forge (lookup). "
        ...
        "seed_sources_note": (
            ...
            "external_tool_intel_sources.json; CVEs em firewallxpl/modules exceto IDs já "
    ...
    out = root / "firewallxpl" / "resources" / "catalogs" / "cve_extended_catalog.json"
